...
export class ChromaAdapter implements VectorStoreAdapter {
  ...
  async connect(): Promise<void> {
    const { mode, host, port, ssl, headers, persistDirectory, tenant, database } = this.config;

    if (mode === "embedded") {
      // Ephemeral in-process client — useful for tests
      const { EphemeralClient } = await import("chromadb");
      this.client = await EphemeralClient({ path: persistDirectory });
    } else {
  ...
  async upsert(request: UpsertRequest): Promise<UpsertResponse> {
    ...
    try {
      collection = await client.getOrCreateCollection({ name: request.collectionName });
    } catch (err) {
      throw new AdapterError(
  ...
  async query(request: QueryRequest): Promise<QueryResponse> {
    ...
    const queryParams: Parameters<Collection["query"]>[0] = {
      ...
      include: ["documents", "metadatas", "embeddings", "distances"] as never,
    };
    if (request.where) queryParams.where = request.where as never;
    if (request.whereDocument)
      queryParams.whereDocument = { $contains: request.whereDocument } as never;

    let raw: Awaited<ReturnType<Collection["query"]>>;
    try {
      raw = await collection.query(queryParams);
    } catch (err) {
    ...
    const embList = (raw as Record<string, unknown[][]>)["embeddings"]?.[0] ?? [];
  ...
  async createCollection(request: CreateCollectionRequest): Promise<CreateCollectionResponse> {
    const client = this.assertConnected();
    let existing: Awaited<ReturnType<ChromaClient["listCollections"]>>;
    try {
    ...
    const alreadyExists = existing.some((c) => c.name === request.collectionName);
    try {
  ...
  async dropCollection(request: DropCollectionRequest): Promise<DropCollectionResponse> {
    ...
    if (!existing.some((c) => c.name === request.collectionName)) {
  ...
  async listCollections(): Promise<ListCollectionsResponse> {
    const client = this.assertConnected();
    let raw: Awaited<ReturnType<ChromaClient["listCollections"]>>;
    try {
      raw = await client.listCollections();
    } catch (err) {
      throw new AdapterError(`ChromaDB listCollections failed: ${String(err)}`, "chromadb", true);
    }
