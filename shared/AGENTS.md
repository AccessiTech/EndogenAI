...
Before performing any multi-step contract or validation task interactively, check `scripts/schema/` for an existing script.
Schema validation, type code-generation, and migration checks are all encoded as scripts — extend them, don't repeat steps by hand.
