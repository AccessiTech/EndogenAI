"""Tests for the affective A2A server.
...
import affective.server as _srv
from affective.server import agent_card, dispatch_task, health
...
class _FakeHandler:
    """Stand-in for Affective A2AHandler in unit tests."""
...
async def test_jsonrpc_plain_dispatch(client: AsyncClient) -> None:
    resp = await client.post(
        "/tasks", json={"task_type": "evaluate_signal", "signal": {"value": 0.8}}
    )
    assert resp.status_code == 200
    body = resp.json()
    assert body["result"]["task_type"] == "evaluate_signal"
...
async def test_jsonrpc_envelope_dispatch(client: AsyncClient) -> None:
    ...
    resp = await client.post(
        ...
        json={
            ...
            "id": rpc_id,
            "method": "tasks/send",
            "params": {"task_type": "get_drive_state"},
        },
    )
    assert resp.status_code == 200
    body = resp.json()
    assert body["id"] == rpc_id
    assert body["result"]["task_type"] == "get_drive_state"
...
async def test_no_handler_returns_503() -> None:
    ...
    async with AsyncClient(
        ...
        resp = await c.post("/tasks", json={"task_type": "evaluate_signal"})
...
async def test_integration_health_full_lifespan() -> None:
    ...
    from affective.server import app as prod_app
