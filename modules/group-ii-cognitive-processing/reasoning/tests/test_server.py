"""Tests for the reasoning A2A server.
...
lifespan with real ChromaDB / Ollama connections.
"""
...
import reasoning.server as _srv
from reasoning.server import agent_card, dispatch_task, health
...
class _FakeHandler:
    """Stand-in for Reasoning A2AHandler in unit tests."""
...
async def test_jsonrpc_plain_dispatch(client: AsyncClient) -> None:
    resp = await client.post(
        "/tasks", json={"task_type": "infer", "prompt": "What is 2+2?"}
    )
    assert resp.status_code == 200
    body = resp.json()
    assert body["result"]["task_type"] == "infer"
...
async def test_jsonrpc_envelope_dispatch(client: AsyncClient) -> None:
    ...
    resp = await client.post(
        ...
        json={
            ...
            "id": rpc_id,
            "method": "tasks/send",
            "params": {"task_type": "plan", "goal": "test goal"},
        },
    )
    assert resp.status_code == 200
    body = resp.json()
    assert body["id"] == rpc_id
    assert body["result"]["task_type"] == "plan"
...
async def test_no_handler_returns_503() -> None:
    ...
    async with AsyncClient(
        ...
        resp = await c.post("/tasks", json={"task_type": "infer"})
...
async def test_integration_health_full_lifespan() -> None:
    ...
    from reasoning.server import app as prod_app
