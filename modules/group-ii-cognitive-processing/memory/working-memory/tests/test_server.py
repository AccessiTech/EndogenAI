"""Tests for the working-memory A2A server.
...
lifespan with real ChromaDB / Redis / Ollama connections.
"""
...
import working_memory.server as _srv
from working_memory.server import agent_card, dispatch_task, health
...
class _FakeHandler:
    """Stand-in for WorkingMemory A2AHandler in unit tests."""
...
async def test_jsonrpc_plain_dispatch(client: AsyncClient) -> None:
    resp = await client.post("/tasks", json={"task_type": "ping", "x": 1})
    assert resp.status_code == 200
    body = resp.json()
    assert body["result"]["task_type"] == "ping"
...
async def test_no_handler_returns_503() -> None:
    ...
    async with AsyncClient(
        ...
        resp = await c.post("/tasks", json={"task_type": "ping"})
...
async def test_integration_health_full_lifespan() -> None:
    """Health endpoint on fully-initialised server (requires live services)."""
    from working_memory.server import app as prod_app
