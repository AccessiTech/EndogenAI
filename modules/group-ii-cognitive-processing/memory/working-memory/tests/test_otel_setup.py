"""test_otel_setup.py — Unit tests for working-memory OTel configuration.
...
import working_memory.instrumentation.otel_setup as _otel
...
def test_configure_telemetry_idempotent() -> None:
    ...
    _otel.configure_telemetry()
    # Patch set_tracer_provider so we can confirm it is NOT called a second time
...
def test_add_otel_trace_context_no_active_span() -> None:
    """Without an active span the processor returns event_dict unchanged."""
    event_dict: dict = {"event": "test_event"}
    result = _otel._add_otel_trace_context(None, "info", event_dict)
    assert result["event"] == "test_event"
    # No trace_id/span_id injected when no active span
...
def test_add_otel_trace_context_with_active_span() -> None:
    ...
    assert len(result["span_id"]) == 16   # 16 hex chars


def test_add_otel_trace_context_suppresses_import_error() -> None:
    """If opentelemetry is unavailable, the processor still returns event_dict."""
