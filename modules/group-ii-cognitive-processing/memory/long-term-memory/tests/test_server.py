"""Tests for the long-term-memory A2A server.
...
lifespan with real ChromaDB / Kuzu connections.
"""
...
import long_term_memory.server as _srv
from long_term_memory.server import agent_card, dispatch_task, health
...
class _FakeHandler:
    """Stand-in for LongTermMemory A2AHandler in unit tests."""
...
async def test_jsonrpc_plain_dispatch(client: AsyncClient) -> None:
    resp = await client.post("/tasks", json={"task_type": "write_item", "content": "fact"})
    assert resp.status_code == 200
    body = resp.json()
    assert body["result"]["task_type"] == "write_item"
...
async def test_jsonrpc_envelope_dispatch(client: AsyncClient) -> None:
    ...
    resp = await client.post(
        ...
        json={
            ...
            "id": rpc_id,
            "method": "tasks/send",
            "params": {"task_type": "retrieve", "query": "test"},
        },
    )
    assert resp.status_code == 200
    body = resp.json()
    assert body["id"] == rpc_id
    assert body["result"]["task_type"] == "retrieve"
...
async def test_no_handler_returns_503() -> None:
    ...
    async with AsyncClient(
        ...
        resp = await c.post("/tasks", json={"task_type": "retrieve"})
...
async def test_integration_health_full_lifespan() -> None:
    ...
    from long_term_memory.server import app as prod_app
